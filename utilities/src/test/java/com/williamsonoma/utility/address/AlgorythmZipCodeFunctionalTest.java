 ...
 * Date: 7/7/2018
 *
 * Functional tests and exception conditions.
 */
@RunWith(SpringRunner.class)
@ContextConfiguration
@SpringBootTest
public class AlgorythmZipCodeFunctionalTest
{
    @Configuration
    static class Config
    {
    ...
    private static final Logger logger = LogManager.getLogger(AlgorythmZipCodeFunctionalTest.class);

    @Test
    public void compactOneToOne()
    {
        logger.info("Test compacting one range into one range successfully");
        var ranges = Arrays.asList(new ZipCodeRange("00010", "00015"));
    ...
    @Test(expected = IllegalArgumentException.class)
    public void compactRangesWithNonNumericLowerZipCode()
    {
        logger.info("Test non-numeric zip code in lower bound expecting IllegalArgumentException");
        var ranges = Arrays.asList(
               new ZipCodeRange("00000", "+0005"),
               new ZipCodeRange("0007A", "00009"));
        printRanges("   ranges before", ranges);
    ...
    @Test(expected = IllegalArgumentException.class)
    public void compactRangesWithNonNumericUpperZipCode()
    {
        logger.info("Test non-numeric zip code in upper bound expecting IllegalArgumentException");
        var ranges = Arrays.asList(
               new ZipCodeRange("00000", "0005B"),
               new ZipCodeRange("00007", "00009"));
        printRanges("   ranges before", ranges);
    ...
    public void compactRangesWithNull()
    {
        ...
        var result = zipCode.compactRanges(null);
        printRanges("   ranges after", result);
